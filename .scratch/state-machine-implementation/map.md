...
- **[13 Prototype the compiled-graph package](issues/13-prototype-the-compiled-graph-package.md)** —
  **resolved.** The prototype establishes `lifecycle.graph` as the pure, immutable projection module
  owned separately from JPA and evaluation; `RequiredAuthority` moves there while `DefinitionScope`
  stays package-private in `definition`. Definition compilation accepts already-materialized entities,
  binds typed Guard and Action params once, and produces evaluator-ready indexes and effective Guard
  chains. One stateless evaluator path serves direct evaluation and Possible Events in authority →
  Information Requirement → Guard order, reporting ordered Actions without running them. The cache is
  keyed only by Definition Version row id. No repository source or production caller landed, so the
  no-I/O and inertness gates remain intact. Prototype judgement calls are recorded in
  [the separate artifact](../compiled-graph-prototype/judgement-calls.md).
